...
from config import LINEONE_Y, THIN_LINETWO_Y, THIN_LINETHREE_Y, DISPLAY_WIDTH, DISPLAY_HEIGHT, THIN_LINE_HEIGHT, THIN_CHAR_WIDTH, offline_mode
import main

async def scroll_text(oled, text, y, speed=5): # Speed is 1-5, 1 being slowest
    # print(f"scroll_text() called with text: {text}")
    text_width = len(text) * THIN_CHAR_WIDTH
    step_size = 6
...
async def test(oled1, oled2):
    ...
    while True:
        await asyncio.gather(
            scroll_text(oled1, test_string, THIN_LINETWO_Y),
            scroll_text(oled2, test_string, THIN_LINETWO_Y)
        )
