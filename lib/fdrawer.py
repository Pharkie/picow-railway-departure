...
class FontLoader:
    def __init__(self, font_name):
        ...
        fsize = [
            items[3] for items in os.ilistdir() if items[0].upper() == fname.upper()
        ]
        if len(fsize) < 1:
    ...
    def print_char(self, char, x, y):
        ...
        if data is None:
            self.fb.rect(
                x,
                y,
                self._font.height * self._fontscale,
                chrwidth * self._fontscale,
                self._fontcolor,
            )
        else:
            # Debug: print( data, x, y, chrwidth, self._font.height, self._fontscale )
            self._fill_bicolor(
                data, x, y, chrwidth, self._font.height, scale=self._fontscale
            )
